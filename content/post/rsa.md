...
这里因为是演示，我们使用比较小的数，选择 p = 7 以及 q = 11，所以 n = 77。

第二步，计算 n 的欧拉函数 $\phi(n)$。

根据公式，$\phi(n) = (p-1)(q-1)$，我么可以得知 $\phi(77) = 6 \times 10 = 60$。
...
在我们的例子中，$\phi(n) = 60$，我们可以很容易的查表得出 $\phi(60) = 16$。
...
在我们的情况中，e 和 $\phi(n)$ 是互质的，所以这个方程式有解。同时，通过扩展欧几里得算法，可以非常容易的通过迭代求解出 d = -23。

d 加减 $\phi(n)$ 的倍数都是有效的值，所以我们加上 60 得到 d = 37

到这里计算完毕，公钥就是 $[n, e] = [77, 13]$，私钥就是 $[n, d] = [77, 37]$。
