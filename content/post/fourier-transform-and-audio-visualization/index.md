...
印象中使用的第一个 PC 音乐播放器就是「千千静听」，大概是 08 年左右。我还清楚地记得它自带了一首梁静茹的歌「Love is everything」，动听的旋律至今萦绕耳旁。

![](qianqian.jpeg)

千千静听的左上角有一组随着音乐跳动的柱子，我想大家都习以为常了，很多播放器都有这功能。但是其实有没有想过，这里是怎么实现的？
...
它的网页做得很棒，通过互动的形式讲述基本的乐理知识，包括音阶、和谐，十二平均律等。
...
想要存储，我们就需要将它离散化，变成离散序列，也就是采样，使用固定的间隔对函数进行求值。
...
而 WAV 则是无损的，输入是什么，读出来的就是什么，缺点自然是体积要大很多。
...
这里关于傅里叶变换的说法并不完整，比如傅里叶变换的输入也可以是复数。但是对我来说理解到这一层面就足够了。如果想要深入学习，可以去搜索数字信号处理相关的知识。
...
因为输出结果是对称的，只要一半的信息有价值，因此有一个变体叫做 `rfft`，只返回一半的信息，计算速度可以更快一些。
...
现在剩下的问题则是输入是什么？我们不可能把一首歌曲的所有样本作为输入进行傅里叶变换，如果这样做，我们只能得到单独一份频率数据。
...
具体的机理我并不理解，所以就不再说明了，对我来说，这部分当做黑盒处理就够了。
...
我在 [audio-vis-demo][demo] 中也基于 WebAudio 的 API 进行了实现，主要目的是为了对比和参考。
