...
根据公式，$\phi(n) = (p-1)(q-1)$，我么可以得知 $\phi(21) = 2 \times 6 = 12$。
...
在我们的例子中，$\phi(n) = 12$，我们可以很容易的查表得出 $\phi(12) = 4$。
...
125 加减 12 的倍数都是有效的模反元素，这里我们取 $d = 125 - 12 \times 9 = 17$。

到这里计算完毕，公钥就是 $[n, e] = [21, 5]$，私钥就是 $[n, d] = [21, 17]$。

## 加密和解密

现在我们来看看加密和解密过程是怎样的。

加密消息 m 需要是一个小于 n 的整数（我们可以将任意字节流直接解读为一个无符号整数），后面我们会说明如果 m 不小于 n 该怎么处理。
...
以 m = kp 为例，此时 k 必然与 q 互质。因为 n = pq，而 m < n，从所以 k 必然小于 q，而 q 是一个质数，在小于 q 的数字当中所有数都与 q 互质。

同事 kp 必然也与 q 互质，如果 kp 和 q 不互质，那么 kp 必然是 q 的倍数，因为 q 不存在其他因子，那么 kp 就是 n 的倍数，因为 n = pq，但是我们的前提是 m < n。
...
因为 n 是公开的，所以私钥的安全本质上就是保证 d 的安全，那么有没有可能在得知 n 和 e 的情况下，推导得出 d？

- 因为 $ed \equiv 1 \pmod {\phi(n)}$，想知道 d 需要知道 e 和 $\phi(n)$
- 因为 e 是公开的，所以想知道 d 只要知道 $\phi(n)$ 即可
- 而 $\phi(n) = (p - 1)(q - 1)$ 计算 $\phi(n)$ 需要对正数 n 进行质数分解

所以，d 的安全性依赖于对 n 进行质数分解的难度。目前来说，大整数的质数分解是一件相当困难的事情，参考维基百科 [整数分解](https://zh.wikipedia.org/wiki/%E6%95%B4%E6%95%B0%E5%88%86%E8%A7%A3)。

## 秘钥格式

一般我们使用 RSA 都是通过 `ssh-keygen` 指令来生成公钥和私钥，默认保存目录为 `~/.ssh`，公钥为 `~/.ssh/id_rsa.pub`，私钥为 `~/.ssh/id_rsa`。

我们现在来生成一对秘钥看看他们的格式是怎样的，上面的几个关键数字 n, e, d 是怎样保存的。
...
这个格式是 OpenSSH 公钥格式，[RFC 4253] 中有详细的说明。简单来说，公钥分为三个部分

- 秘钥类型：`ssh-rsa`
- PEM 编码的一段数据：`AAAA..b1J5`
- 备注：`cj@CJs-Macbook-Pro.local`

PEM 的全称是 [Privacy Enhanced Mail]，是一种 Base64 的编码，使用 ASCII 来编码二进制数据。
...
我们得到了一堆数字，对照如下的说明，就可以知道哪个数字是哪个。
...
清楚了公钥私钥格式以后，我们就可以正式使用 RSA 算法来进行加密了。

因为加密的消息 m 必须要小于 n，所以，在日常应用中，RSA 不会用来直接加密消息，而是加密一个对称秘钥，消息根据这个对称秘钥使用对称加密算法来加密。

这里作为演示，我们直接使用 RSA 来加密一个短的消息。
...
上面我们提到的 RSA 加密过程，也就是 $m^e \equiv c \pmod n$，也被称为教科书式 RSA。正式工程应用中，不会直接这样处理，而是会存在一个 Padding 的过程，具体不再展开，感兴趣可以去看 [RSA - theory and implementation](https://eli.thegreenplace.net/2019/rsa-theory-and-implementation/)。
...
[rsademo] 是我使用 Go 实现的一个 RSA 实现。可以解析 openssh 的秘钥文件以及演示 RSA 加解密，具体功能可以查看 GitHub 的 README。

使用 `rsademo` 加密解密需要提供两个质数，可以使用 [这个网站](https://www.browserling.com/tools/prime-numbers) 生成。
...
[RFC 4253]: https://tools.ietf.org/html/rfc4253#section-6.6
