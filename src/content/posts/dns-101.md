...
使用 `dig` 获取 `cjdev.me` 的 IPv4 地址。

```bash
$ dig cjdev.me # 记录类型默认为 `A`
; <<>> DiG 9.8.3-P1 <<>> cjtinc
;; global options: +cmd
;; Got answer:
...
;; ANSWER SECTION:
cjdev.me.    598 IN  A 192.30.252.153
cjdev.me.    598 IN  A 192.30.252.154
...
可以看到，`cjdev.me` 这个域名有两条 `A` 记录，即查询到了两条 IP 地址。

第二个指令是 `host` 指令，相比 `dig` 指令，功能比较简单，输出也比较简洁。

使用 `host` 获取 `cjdev.me` 的相关信息。

```bash
$ host cjdev.me
cjdev.me has address 192.30.252.154
cjdev.me has address 192.30.252.153
```
...
下面我们来看一个完整的例子，`dig +trace cjdev.me`，看看到底发生了哪些请求，最终的IP地址是如何得到的。

```bash
$ dig +trace cjdev.me
; <<>> DiG 9.8.3-P1 <<>> +trace cjdev.me @8.8.8.8
;; global options: +cmd
...
cjdev.me.              86400   IN      NS      f1g1ns2.dnspod.net.
cjdev.me.              86400   IN      NS      f1g1ns1.dnspod.net.
;; Received 81 bytes from 199.253.60.1#53(199.253.60.1) in 361 ms

cjdev.me.              600     IN      A       192.30.252.154
cjdev.me.              600     IN      A       192.30.252.153
cjdev.me.              86400   IN      NS      f1g1ns1.dnspod.net.
cjdev.me.              86400   IN      NS      f1g1ns2.dnspod.net.
...
7. 192.168.0.100 -> 199.253.60.1: `cjdev.me` 的 A 记录是什么？
8. 199.253.60.1 -> dig: `cjdev.me` 的 NS 记录是 `f1g1ns1.dnspod.net` 和 `f1g1ns2.dnspod.net`
...
10. 192.168.0.100 -> 180.163.19.15: `cjdev.me` 的 A 记录是什么？
